...
from models_pytorch import LiteECGCNN
...
def main():
    ...
    model = LiteECGCNN(input_length=input_length, num_classes=num_classes).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=1e-3)

    num_epochs = 20
    n_params = count_parameters(model)
    print(f"LiteECGCNN parameters: {n_params}")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print(f"[LiteECGCNN] Epoch {epoch:02d} | "
              f"train_loss={train_loss:.4f}, train_acc={train_acc:.4f} | "
              f"val_loss={val_loss:.4f}, val_acc={val_acc:.4f}")

    end_time = time.time()
    total_train_time = end_time - start_time
    print(f"\n[LiteECGCNN] Total training time: {total_train_time:.2f} seconds")

    # 6. Final evaluation
    val_loss, val_acc, targets, preds = eval_one_epoch(model, val_loader, criterion, device)
    print("\n[LiteECGCNN] Final validation loss:", val_loss)
    print("[LiteECGCNN] Final validation accuracy:", val_acc)
    ...
    print("\n[LiteECGCNN] Confusion matrix:\n", cm)

    report = classification_report(targets, preds, output_dict=True)
    print("\n[LiteECGCNN] Classification report:\n")
    ...
    # 8. Save plots (PNG) – LiteECGCNN only[web:675][web:676]
    ...
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.set_title('MIT-BIH all – LiteECGCNN loss')
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_title('MIT-BIH all – LiteECGCNN accuracy')
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_ylabel('True')
    ax.set_title('MIT-BIH all – LiteECGCNN confusion matrix')
    fig.tight_layout()
    cm_png = os.path.join(PLOTS_DIR, f"{prefix}_confusion.png")
    fig.savefig(cm_png, dpi=300, bbox_inches='tight')
    plt.close(fig)

    print("\n[LiteECGCNN] Saved CSV tables in:", RESULTS_DIR)
    print("[LiteECGCNN] Saved plots in:", PLOTS_DIR)
