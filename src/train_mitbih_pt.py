...
#for google colab, project root is changed from  r"F:\ecg_thesis" to import os
...
def main():
    ...
    model = BaselineCNN(input_length=input_length, num_classes=num_classes).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=1e-3)

    num_epochs = 20
    n_params = count_parameters(model)
    print(f"BaselineCNN parameters: {n_params}")
    ...
    history_csv = os.path.join(RESULTS_DIR, "mitbih_all_baseline_history.csv")
    history_df.to_csv(history_csv, index=False)

    cm_df = pd.DataFrame(cm, index=class_names, columns=class_names)
    cm_csv = os.path.join(RESULTS_DIR, "mitbih_all_baseline_confusion.csv")
    cm_df.to_csv(cm_csv)

    report_df = pd.DataFrame(report).transpose()
    report_csv = os.path.join(RESULTS_DIR, "mitbih_all_baseline_classification_report.csv")
    ...
    summary_csv = os.path.join(RESULTS_DIR, "mitbih_all_baseline_summary.csv")
    ...
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.set_title('MIT-BIH all records – BaselineCNN loss')
    ax.legend()
    fig.tight_layout()
    loss_png = os.path.join(PLOTS_DIR, "mitbih_all_baseline_loss.png")
    ...
    ax.set_title('MIT-BIH all records – BaselineCNN accuracy')
    ax.legend()
    fig.tight_layout()
    acc_png = os.path.join(PLOTS_DIR, "mitbih_all_baseline_accuracy.png")
    ...
    ax.set_ylabel('True')
    ax.set_title('MIT-BIH all records – BaselineCNN confusion matrix')
    fig.tight_layout()
    cm_png = os.path.join(PLOTS_DIR, "mitbih_all_baseline_confusion.png")
    fig.savefig(cm_png, dpi=300, bbox_inches='tight')
    plt.close(fig)

    print("\nSaved CSV tables in:", RESULTS_DIR)
